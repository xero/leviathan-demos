 ...
 *   44+:   pool output (SerpentStream wire format)
 */
...
export const OFF_CIPHER   = 44;  // pool output starts here
...
export interface Srpt256sHeader {
	...
	// streamHeader removed — carried inside poolOutput
}

/**
 * Encode a complete SRPT256S blob (outer header + pool output) into a Uint8Array.
 */
export function encodeBlob(header: Srpt256sHeader, poolOutput: Uint8Array): Uint8Array {
	...
	buf.set(poolOutput, OFF_CIPHER);
	return buf;
}

/**
 * Parse and validate a raw binary SRPT256S blob.
 * Returns the parsed header and pool output, or throws a descriptive Error.
 */
export function decodeBlob(
	blob: Uint8Array,
): { header: Srpt256sHeader; poolOutput: Uint8Array } {
	if (blob.length < HEADER_SIZE) throw new Error('File too short to be a valid SRPT256S file');
	...
	return {
		header:     { version, flags, kdf, salt: blob.slice(OFF_SALT, OFF_SALT + 32) },
		poolOutput: blob.slice(OFF_CIPHER),
	};
}
