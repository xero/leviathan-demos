# lvthn-tamper wire protocol

> [!NOTE]
> Version 1.0. This document specifies the complete wire protocol for
> lvthn-tamper. All cryptographic processing happens in the browser; the relay
> server is a dumb pipe that never inspects message content beyond routing
> metadata.
...
```
dh      = X25519.dh(my_sk, peer_pk)              // 32-byte shared secret
info    = TextEncoder.encode("lvthn-tamper/x25519-xchacha20")
key     = HKDF_SHA256.derive(dh, null, info, 32) // 32-byte AEAD key
nonce   = randomBytes(24)                         // fresh per message
...
The raw Diffie-Hellman output is not used as the cipher key directly. It runs
through HKDF-SHA256 (RFC 5869) with a fixed `info` label, which produces a
uniformly distributed, domain-separated key. The salt is null, which is fine
here because each session uses a fresh ephemeral X25519 keypair. Both peers
feed the identical `info` label, so the derived key matches on both sides.
`X25519.dh()` also rejects an all-zero shared secret, blocking small-order
peer public keys per RFC 7748 §7.
