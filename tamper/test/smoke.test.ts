/**
 * smoke.test.ts: Playwright end-to-end tests for lvthn-tamper
 *
...
test.describe('Setup phase', () => {
	...
	test('keypair panel shows public key and protects private key after connecting', async ({ page }) => {
		...
		// Private key is never displayed; the panel shows a status note instead
		const privKeyText = await page.locator('.keypair-panel').textContent();
		expect(privKeyText).toContain('held in memory for this session only, never transmitted');
