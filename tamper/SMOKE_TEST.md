# lvthn-tamper smoke test checklist

> [!NOTE]
> Manual verification steps for the tamper demo. Run after any change to the
> client or server.
...
1. `cd tamper && bun bake` builds `dist/index.html`
2. `bun run server/server.ts` starts the relay on ws://localhost:3000
3. Open `dist/index.html` in two separate browser windows (Window A and Window B)
